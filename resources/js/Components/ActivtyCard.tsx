...
const ActivityCard = ({ activities, user }: ActivityProps) => {
    ...
    const getRecommendation = async () => {
        try {
            ...
            const parsedResult: { category: string[], reason: string } = JSON.parse(result);
            console.log(parsedResult);

            setSelectedCategories(parsedResult.category);
            setMessage(parsedResult.reason);
        } catch (error) {
            console.log(error);
        } finally {
            setLoading(false);
        ...
        <section>
            <div className="max-w-6xl mx-auto py-12 md:py-20">
                <header className="py-8 text-center">
                    ...
                    <div className="mt-6 flex justify-center gap-4">
                        ...
                        {user?.role === "personal" && (
                            <Button
                                ...
                                disabled={loading}
                                tooltipContent="Pilih aktivitas berdasarkan rekomendasi Google Gemini"
                            >
                    ...
                    {
                        message && (
                            <Alert className="w-3/4 z-50 mx-auto mt-6 text-start border-primary">
                                <SiGooglegemini className="h-4 w-4" />
                                <AlertTitle>Berikut rekomendasi dari kami :</AlertTitle>
                                <AlertDescription>
                                    {message}
                                </AlertDescription>
                                ...
                                <Button
                                    ...
                                    className="absolute top-2 right-2 rounded-full hover:bg-background flex items-start"
                                    aria-label="Close"
                                >
                                    <XIcon />
                                </Button>
